...
MoonBit 生态仍在快速建设阶段，Graph 与 Pathfinding 属于赛题推荐方向，也是大量上层工具的公共底座。本项目采用原创 MoonBit 实现，优先保证 API 清晰、行为可复现、测试覆盖和持续集成，并已扩展到 JSON/text 交换格式、双向寻路、生成式性质测试和性能基准。

## 核心功能范围

- 提供 `Graph[N]` 加权有向图模型，支持节点、边、无向边、删边、删节点、邻接查询、边存在查询、入度/出度、节点数与边数统计、边权摘要、边列表导出、边列表重建、安全批量构建和诱导子图。
- 提供 BFS 与 DFS，用于无权层次路径、深度遍历和图结构探索。
- 提供 Dijkstra、bidirectional Dijkstra、Path 节点/边数统计、显式路径成本校验、单源距离表和全点距离表，用于非负权重图最短路径与批量距离查询。
- 提供 A*，支持调用方注入启发函数。
...
- 提供 `Grid`、`Point` 和 `CellCost`，支持 blocked cells、terrain costs、blocked/terrain export、grid rebuild、grid resizing、rectangular bulk updates、line-of-sight、path smoothing、grid path validation、path cost recomputation、4 邻接、8 邻接、禁止穿角的 8 邻接 tile-grid 寻路和 heuristic-free grid Dijkstra。
...
第二阶段：完成 BFS、DFS、Dijkstra、bidirectional Dijkstra、A*、网格寻路、拓扑排序、拓扑分层、单源/全点距离表、连通性分析和强/弱连通分量。
...
- JSON/text 导入导出，便于与可视化前端交换图结构。
- 双向 Dijkstra / 双向 A*。
- 生成式图与 grid 性质测试。
- 覆盖 grid、sparse DAG、dense graph 的 benchmark 和 timing 摘要。
- 形成 `v0.30.1` 版本标签、GitHub Release 和最终提交说明。
