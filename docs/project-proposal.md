...
- 提供 Dijkstra、bidirectional Dijkstra、显式路径成本校验、单源距离表和全点距离表，用于非负权重图最短路径与批量距离查询。
- 提供 A*，支持调用方注入启发函数。
...
- 提供 `Grid`、`Point` 和 `CellCost`，支持 blocked cells、terrain costs、blocked/terrain export、grid rebuild、rectangular bulk updates、line-of-sight、path smoothing、4 邻接、8 邻接、禁止穿角的 8 邻接 tile-grid 寻路和 heuristic-free grid Dijkstra。
