...
MoonPath 面向 MoonBit 生态提供轻量、可复用、可测试的图算法与网格寻路基础库。项目服务于游戏地图、机器人/路径规划、流程编排、依赖图分析、可视化工具和编译器中间结构遍历等场景，提供统一的加权图模型、常用遍历算法、最短路算法、A* 搜索、DAG 分层分析和 tile-grid 辅助能力。
...
- 提供 BFS，用于无权最短层数路径。
- 提供 Dijkstra 和单源距离表，用于非负权重图最短路径与批量距离查询。
- 提供 A*，支持调用方注入启发函数。
- 提供 reachability、weakly connected components、strongly connected components、graph transpose、topological sort、topological layers 和 acyclic checks。
- 提供 `Grid` 和 `Point`，支持 blocked cells、terrain costs、4 邻接、8 邻接和禁止穿角的 8 邻接 tile-grid 寻路。
...
第二阶段：完成 BFS、Dijkstra、A*、网格寻路、拓扑排序、拓扑分层、单源距离表、连通性分析和强/弱连通分量。

第三阶段：补充 weighted terrain、no-corner-cutting pathfinding、grid-to-graph、文档、示例、CI、开发报告和验收清单，公开同步 GitHub 与 GitLink 仓库。
