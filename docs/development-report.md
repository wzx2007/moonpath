...
- 实现 grid blocked cells、terrain costs、point enumeration、open cell enumeration、blocked/terrain export、grid rebuild、rectangular bulk updates 和 grid-to-graph 转换。
- 实现二叉堆优先队列作为最短路算法内部结构。
- 编写 20 个黑盒测试，覆盖最短路、双向最短路、显式路径成本、边列表重建、诱导子图、单源/全点距离、BFS、DFS、拓扑排序、拓扑分层、图结构分析、强/弱连通分量、连通判定、grid A*、grid Dijkstra、grid export/rebuild、terrain costs、rectangular updates、corner cutting、grid-to-graph 和启发函数。
