...
- 编写 23 个黑盒测试，覆盖最短路、双向最短路、显式路径成本、图动态更新、边列表重建、安全批量构建、诱导子图、单源/全点距离、BFS、DFS、拓扑排序、拓扑分层、图结构分析、强/弱连通分量、连通判定、grid A*、grid Dijkstra、grid export/rebuild、line-of-sight、path smoothing、terrain costs、rectangular updates、corner cutting、grid-to-graph 和启发函数。
