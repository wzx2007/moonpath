...
- 实现加权有向图建模能力：节点、边、无向边、邻接查询、边存在查询、入度/出度、节点数、边数统计、边列表导出、从边列表重建和诱导子图。
- 实现 BFS、DFS preorder/postorder、Dijkstra、bidirectional Dijkstra、A*、explicit path-cost scoring、single-source distance map、all-pairs distance map、shortest-distance query、path-existence query、reachable、graph eccentricity 和 graph diameter。
...
- 实现 grid blocked cells、terrain costs、point enumeration、open cell enumeration、blocked/terrain export、grid rebuild、rectangular bulk updates、line-of-sight、path smoothing 和 grid-to-graph 转换。
- 实现二叉堆优先队列作为最短路算法内部结构。
- 编写 21 个黑盒测试，覆盖最短路、双向最短路、显式路径成本、边列表重建、诱导子图、单源/全点距离、BFS、DFS、拓扑排序、拓扑分层、图结构分析、强/弱连通分量、连通判定、grid A*、grid Dijkstra、grid export/rebuild、line-of-sight、path smoothing、terrain costs、rectangular updates、corner cutting、grid-to-graph 和启发函数。
